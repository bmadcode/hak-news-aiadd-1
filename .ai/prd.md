...
- Story 2: Hacker News Retrieval API Route (COMPLETED)
...
- Story 3: Configure and Deploy CDK Stack (COMPLETED)
...
- Story 4: E2E Testing with Playwright (COMPLETED)
...
- Story 5: POST API Route to Get N Summarized Post/Article/Comments results (COMPLETED)
...
- Story 6: Playwright Test for the Summarized Post/Article/Comments API Route (COMPLETED)
...
- Story 7: There should only be a single summarization route, remove the route to get hacker news stories and comments. (COMPLETED)
...
- Story 8: Email Summaries as a News Letter (COMPLETED)
...
- Story 9: use email subscriptions to send the emails (COMPLETED)
...
- Story 10: Cache HTML in DynamoDB & Async Optimization
  ...
  - Task 3: The TST controller will be updated to use the email service to send the emails of the summaryHtml to the email addresses in the row
    ...
    - The email service will send the email to the email addresses defined in the rows with the pk of `sub:hakdaily`
    - The email service function will no longer need to take in json and generate the html, it will just take in the html and send the email, this code can be refactored out of the email service
